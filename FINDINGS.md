...
What we measured while building [RECIPE.md](RECIPE.md), including the negative
results and the bugs we shipped and caught. Every claim names the run that
produced it.

The recipe's headline claims come from F1 (token budget), F7 (evidence layout),
F13 (fine-tuning works), F14 (anchor-alignment transfers) and F16 (a 22M
cross-encoder beats us on quality).
...
queries, identical BM25 top-100 pool, identical 2,000-char truncation, paired
bootstrap over queries.

| system | params | nDCG@10 | vs BM25 | 95% CI | p | ECE raw → cal | ms/query |
|---|---|---|---|---|---|---|---|
| **MiniLM-L6 cross-encoder** | **22.7M** | **0.7197** | **+0.153** | [+0.068, +0.240] | <0.001 | 0.326 → 0.192 | 599 |
| Laya tuned (anchored) | 422.3M | 0.6229 | +0.056 | [−0.009, +0.119] | **0.088** | **0.219 → 0.111** | 4474 |
| BM25 (first stage) | — | 0.5672 | — | — | — | — | 0 |
| Laya base (anchored) | 421.3M | 0.4133 | −0.154 | [−0.216, −0.094] | <0.001 | n/a | 2572 |

**Quality: the cross-encoder wins decisively.** +0.097 nDCG@10 over the
fine-tuned System One model, at 1/19th the parameters and 7× the speed. Its
gain over the first stage is significant; **ours is not** (p = 0.088, CI crosses
zero). That is after fine-tuning, a custom ordinal head, and a loss the design
document does not have.

**Calibration: the System One model wins, by less than hoped.** ECE 0.111 vs
0.192 *after both* get a temperature fitted on a held-out half of the queries,
so it is not an artifact of MiniLM being trained on a different positive rate.
1.7× better calibration is real but does not buy +0.097 nDCG@10 and 7× latency.

ECE is reported raw and calibrated because raw ECE largely measures whether a
model's training corpus had a similar positive rate to yours, which is not a
property of the model.

**Consequence for the recipe.** Fine-tune a System One model when you need a
calibrated probability from a small self-hosted model on your own decision
boundary. If you need the highest ranking number, use a cross-encoder. This is
stated in README and RECIPE rather than buried here.

Note also that the base checkpoint sits *below* BM25 on a realistic pool
(0.413 vs 0.567), which is the concrete form of "not a zero-shot decision
engine".
