...
class TrainedScorer:
    """Adapts a `SystemOneModel` to `ScorerProtocol` so eval reuses AnchoredScorer.
    ...
    def choice_logprobs(self, instructions, options, state):
        from ..reranking.packing import PackedSlate
...
def make_eval_fn(scorer, ds, index, sigbuilder, packer, anchor_pool, query_ids,
    ...
    """
    from ..reranking.scoring import AnchoredScorer
