...
__all__ = ["delta_ndcg_weights", "lambda_ranknet_loss", "listnet_loss",
           ...
           "TypedDecisionLoss", "typed_ce", "typed_rps", "confidence_penalty"]
...
# ==========================================================================
# The generic typed-decision objective
# ==========================================================================
#
# Everything above is reranking-shaped. This is the loss for ordinary
# fine-tuning: you have typed questions and labels, and you want a model whose
# probabilities mean something.
#
# Three terms, and the second is the one people leave out:
#
#   ce    cross-entropy against the target distribution. Handles hard labels
#         and teacher distributions identically, because a hard label is just
#         a one-hot target -- so distillation costs no extra code path.
#
#   rps   ranked probability score, applied ONLY to `score` questions. RPS is
#         strictly proper AND ordinal-aware: being off by one level costs less
#         than being off by three, which plain cross-entropy has no way to
#         know. Laya's own RLCD applies it at w_rps = 1.0 and it is the single
#         cheapest quality win available on ordinal tasks.
#
#   conf  an optional penalty on over-confidence. System One models ship
#         over-confident (Laya's ECE moves 0.466 -> 0.081 on temperature refit
#         alone), and fine-tuning on hard labels makes it worse. This is not a
#         substitute for the calibration stage -- fit a temperature afterwards
#         regardless -- it just stops training from actively fighting it.

def typed_ce(logits, target, mask=None, weight=None):
    """Cross-entropy against a target distribution, masked to real options.

    Masking matters here for the same reason it did in `listnet_loss`: padded
    marker slots carry -1e4 logits, and any target mass left on them produces
    an enormous loss that is pure padding artefact.
    """
    if mask is not None:
    ...
    per = -(target * logp.masked_fill(~mask.bool(), 0.0) if mask is not None
            else target * logp).sum(-1)
    if weight is not None:
        return (per * weight).sum() / weight.sum().clamp_min(1e-9)
    return per.mean()


def typed_rps(logits, target, mask=None, ordinal=None, weight=None):
    """Ranked probability score over ORDERED levels. Applies to `score` only.

    `ordinal` is a per-example bool; rows that are not ordinal contribute
    nothing, because shuffling a choice's options would make its CDF
    meaningless.
    """
    if mask is not None:
        logits = logits.masked_fill(~mask.bool(), -1e4)
    ...
    if mask is not None:
        ...
        target = target * mask.float()
        target = target / target.sum(-1, keepdim=True).clamp_min(1e-9)
    k = (mask.sum(-1).clamp(min=2).float() if mask is not None
         else torch.full(p.shape[:-1], p.shape[-1], device=p.device).float())
    per = ((torch.cumsum(p, -1) - torch.cumsum(target, -1)) ** 2).sum(-1) / (k - 1)
    if ordinal is not None:
        per = per * ordinal.float()
        denom = ordinal.float().sum().clamp_min(1.0)
    else:
        denom = torch.tensor(float(per.numel()), device=per.device)
    if weight is not None:
        per = per * weight
    return per.sum() / denom


def confidence_penalty(logits, mask=None, target_entropy_frac: float = 0.0):
    """Penalise reported entropy below a floor. Zero by default.

    Raise it only if your eval shows the model collapsing to one-hot answers on
    genuinely ambiguous inputs. The principled fix is a proper scoring rule
    plus a fitted temperature; this is a blunt instrument and is off unless
    asked for.
    """
    if target_entropy_frac <= 0:
    ...
    return F.relu(target_entropy_frac - ent).mean()


class TypedDecisionLoss(torch.nn.Module):
    """The objective for ordinary System One fine-tuning.

        L = w_ce * CE(target) + w_rps * RPS(score questions) + w_conf * penalty

    Defaults mirror Laya's own RLCD weighting, which applies the ordinal rule
    at full strength to score questions and nothing to the others.
    """

    def __init__(self, w_ce: float = 1.0, w_rps: float = 1.0,
                 w_conf: float = 0.0, target_entropy_frac: float = 0.0):
        super().__init__()
        self.w_ce, self.w_rps = w_ce, w_rps
        self.w_conf, self.target_entropy_frac = w_conf, target_entropy_frac

    def forward(self, *, logits, target, mask=None, ordinal=None, weight=None):
        out = {}
        out["ce"] = typed_ce(logits, target, mask, weight)
        total = self.w_ce * out["ce"]
        if self.w_rps and ordinal is not None and bool(ordinal.any()):
            out["rps"] = typed_rps(logits, target, mask, ordinal, weight)
            total = total + self.w_rps * out["rps"]
        if self.w_conf:
            out["conf"] = confidence_penalty(logits, mask, self.target_entropy_frac)
            total = total + self.w_conf * out["conf"]
        out["total"] = total
        return total, out
