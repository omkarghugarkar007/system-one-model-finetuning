"""Check `model.budget`'s arithmetic against the real tokenizer.

`model.budget` predicts, from token counts alone, how much option text, how much
...
from .budget import DEFAULT_BUDGET, SlateLayout, plan_slate_budget
...
def probe_table(tok, **kw) -> str:
    rows = verify(tok, **kw)
    lines = ["Predicted (model.budget) vs measured (real Laya tokenizer)", "",
