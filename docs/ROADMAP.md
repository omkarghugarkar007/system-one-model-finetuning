...
| [~] | **Confirm F7 at full scale with bootstrap CIs** | `make phase0-signal-full` | n=8 → n=50, paired CIs |
| [!] | T4 — does anchoring buy the cross-slate scale? | `make phase0-anchors` | F8 — underpowered (+0.010 ± 0.013) |
| [ ] | Re-run T4 with continuous pivot utilities from Jev | `make phase0-anchors-teacher` | unblocks F8 |
...
| | Task | Command | Gate / output |
|---|---|---|---|
| [ ] | Full reranking pipeline on a realistic top-100 pool | `make rerank` | nDCG@10 comparable to literature |
| [ ] | Ordinal CORAL head (`P(y > k)` on the 0–3 scale) | — | plan Part IX, head 2 |
| [ ] | Retarget the act head to "will a teacher call change top-k?" | — | plan Part IX, head 3 |
| [ ] | Slate dataset: anchors mirror inference, order randomised | — | calibration must transfer |
| [ ] | Hard-negative mining across BM25 + dense + late-interaction | — | plan Part IX — "worth more than the loss" |
| [ ] | Trainer on MPS (grad checkpointing, accumulation) | `make train` | a checkpoint that beats base |
